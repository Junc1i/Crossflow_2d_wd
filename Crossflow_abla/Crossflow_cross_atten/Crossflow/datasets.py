...
class WebDatasetDataset(IterableDataset):
    ...
    def __init__(self, tar_pattern, resolution=256, shuffle_buffer=300, 
        ...
        # 注意：使用分配给当前进程的样本数（self.num_samples），而不是全局样本数
        ...
        if allow_shared_shards:
            ...
            # 使用全局定义的 nodesplitter_identity，避免 pickle 问题
        ...
        pipeline_stages.extend([
            ...
            ),  # 预处理，_preprocess_input 返回 (pixel_values, input_tensor)
            wds.map(self._unpack_input_tuple),  # 将 (pixel_values, input_tensor), output_tensor 展开为三个值
        ])
        ...
        # 限制 epoch 大小（参考 data.py 第412行，在创建 pipeline 之后调用）
        # 这样可以根据计算结果动态设置 epoch 大小
