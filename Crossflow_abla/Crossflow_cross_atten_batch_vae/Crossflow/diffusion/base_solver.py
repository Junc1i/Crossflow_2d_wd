...
class Solver:
    ...
    def sample(
        ...
        **kwargs,
        ...
        # 从 kwargs 中提取 image_latent（如果存在）
    ...
    def get_model_output_dimr(
        ...
        if has_null_indicator:
            # 检查模型类型，DiT模型不需要log_snr参数
            if hasattr(self.model, '__class__') and 'DiT' in self.model.__class__.__name__:
                # DiT模型只使用t参数
                _cond = self.model(x, t=t_continuous, null_indicator=torch.tensor([False] * x.shape[0]).to(x.device), image_latent=image_latent)[-1]
                _uncond = self.model(x, t=t_continuous, null_indicator=torch.tensor([True] * x.shape[0]).to(x.device), image_latent=image_latent)[-1]
            else:
                # DiMR模型使用log_snr参数
                _cond = self.model(x, t=t_continuous, log_snr=log_snr, null_indicator=torch.tensor([False] * x.shape[0]).to(x.device), image_latent=image_latent)[-1]
                _uncond = self.model(x, t=t_continuous, log_snr=log_snr, null_indicator=torch.tensor([True] * x.shape[0]).to(x.device), image_latent=image_latent)[-1]
            ...
            # 检查模型类型
            if hasattr(self.model, '__class__') and 'DiT' in self.model.__class__.__name__:
                _cond = self.model(x, t=t_continuous, image_latent=image_latent)[-1]
            else:
                _cond = self.model(x, log_snr=log_snr, image_latent=image_latent)[-1]
            return _cond
