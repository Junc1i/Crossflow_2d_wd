...
class TransformerBlock(nn.Module):
    ...
    def _forward(self, x, condition_embeds=None, condition_masks=None, pos=None, image_latent=None):
        ...
        if self.use_cross_attention and self.block1_cross is not None and image_latent is not None:
            x = x + self.dropout(self.block1_cross(x, image_latent, pos))
...
class MRModel(nn.Module):
    def __init__(self, config):
        ...
        self.logit_scale = nn.Parameter(torch.ones([]) * np.log(40.0))
        self.visual_token_projection = nn.Linear(2048, config.channels*config.stage_configs[-1].max_height*config.stage_configs[-1].max_width)
        # 添加 Attention Pooling 网络：学习每个token的重要性权重
...
#        print("通过VAE之后的shape:",output.shape)
    ...
    def _visual_tokens_projection(self, visual_tokens, mask=None):
        """
        使用 Attention Pooling 将 visual tokens 投影到 latent space
        
        Args:
            visual_tokens: [B, 576, 2048] - VL模型输出的visual tokens
            mask: [B, 576] - 可选，1表示有效token，0表示padding
        
        Returns:
            projected: [B, latent_size] - 投影后的特征
        """
        # 计算每个token的重要性分数
        attn_weights = self.attention_pool(visual_tokens)  # [B, 576, 1]
        
        # 处理mask（忽略padding token）
        if mask is not None:
            mask_expanded = mask.unsqueeze(-1)  # [B, 576, 1]
            attn_weights = attn_weights.masked_fill(mask_expanded == 0, -1e4)
        
        # 归一化为权重
        attn_weights = F.softmax(attn_weights, dim=1)  # [B, 576, 1]
        
        # 加权平均
        pooled_tokens = (visual_tokens * attn_weights).sum(dim=1)  # [B, 2048]
        
        # 投影到目标维度
        projected = self.visual_token_projection(pooled_tokens)  # [B, latent_size]
        
        return projected
        
    
    def forward(self, x, t = None, log_snr = None, text_encoder=False, text_decoder=False, project_visual_tokens=False, shape=None, mask=None, null_indicator=None, image_latent=None):
        if text_encoder:
        ...
        elif project_visual_tokens:
            return self._visual_tokens_projection(visual_tokens = x, mask=mask)
        else:
