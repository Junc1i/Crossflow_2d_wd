...
class TransformerBlock(nn.Module):
    ...
    def _forward(self, x, condition_embeds=None, condition_masks=None, pos=None, image_latent=None, use_cross_atten_mask=None):
        ...
        if self.use_cross_attention and self.block1_cross is not None and image_latent is not None:
            # 支持mask：决定哪些样本使用cross attention
            if use_cross_atten_mask is not None and use_cross_atten_mask.any():
                # 计算全部样本的cross attention
                cross_attn_output = self.block1_cross(x, image_latent, pos)
                # 应用mask：只有mask为True的样本才加上cross attention结果
                # use_cross_atten_mask shape: [batch_size]
                # cross_attn_output shape: [batch_size, seq_len, hidden_dim]
                mask_expanded = use_cross_atten_mask.view(-1, 1, 1).float()  # [batch_size, 1, 1]
                x = x + self.dropout(cross_attn_output * mask_expanded)
            elif use_cross_atten_mask is None:
                # 推理时：全部使用cross attention
                x = x + self.dropout(self.block1_cross(x, image_latent, pos))
            # else: mask全为False，跳过cross attention
        ...
        if self.has_diff:
            ...
            for stage in self.stages:
                output, lowres_skips = stage(
                    ...
                    use_cross_atten_mask=use_cross_atten_mask  # 新增：传递 use_cross_atten_mask
                )
