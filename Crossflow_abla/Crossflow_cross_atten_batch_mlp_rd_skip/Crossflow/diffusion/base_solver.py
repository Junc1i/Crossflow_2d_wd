    ...
    def get_model_output_dimr(
        ...
        if has_null_indicator:
            ...
            if hasattr(self.model, '__class__') and 'DiT' in self.model.__class__.__name__:
                # DiT模型只使用t参数
                _cond = self.model(x, t=t_continuous, null_indicator=torch.tensor([False] * x.shape[0]).to(x.device), image_latent=image_latent, use_cross_atten_mask=use_cross_atten_mask)[-1]
                _uncond = self.model(x, t=t_continuous, null_indicator=torch.tensor([True] * x.shape[0]).to(x.device), image_latent=image_latent, use_cross_atten_mask=use_cross_atten_mask)[-1]
            else:
                # DiMR模型使用log_snr参数
            ...
            # 检查模型类型
            if hasattr(self.model, '__class__') and 'DiT' in self.model.__class__.__name__:
                _cond = self.model(x, t=t_continuous, image_latent=image_latent, use_cross_atten_mask=use_cross_atten_mask)[-1]
            else:
