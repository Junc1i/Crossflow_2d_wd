...
#        print("para.device:",param.device) 
    ...
    def train_step(_batch, _ss_empty_context):
        ...
        # 生成固定50%比例的mask
        batch_size = _input_image_latent.size(0)
        n_use_cross_atten = batch_size // 2
        use_cross_atten = torch.zeros(batch_size, dtype = torch.bool, device = device)
        use_cross_atten[:n_use_cross_atten] = True
        shuffle_indices = torch.randperm(batch_size, device = device)
        use_cross_atten_mask = use_cross_atten[shuffle_indices]
    ...
    def ode_fm_solver_sample(nnet_ema, _n_samples, _sample_steps, context=None, caption=None, testbatch_img_blurred=None, two_stage_generation=-1, token_mask=None, return_clipScore=False, ClipSocre_model=None, image_latent=None, use_cross_atten_mask=None):
        with torch.no_grad():
            ...
            # 如果没有传入mask，默认全部使用cross attention（None表示不应用mask）
            # 否则使用传入的mask
            if use_cross_atten_mask is None:
                # 推理时默认全部使用cross attention
                pass  # 保持为None

            ode_solver = ODEEulerFlowMatchingSolver(nnet_ema, step_size_type="step_in_dsigma", guidance_scale=_cfg)
            _z, _ = ode_solver.sample(x_T=_z_init, batch_size=_n_samples, sample_steps=_sample_steps, unconditional_guidance_scale=_cfg, has_null_indicator=has_null_indicator, image_latent=image_latent, use_cross_atten_mask=use_cross_atten_mask)
    ...
    while train_state.step < config.train.n_steps:
        ...
        if train_state.step % config.train.eval_interval == 0:
            ...
            if hasattr(dataset, "vis_image_paths"):
                ...
                raise NotImplementedError()
             # 生成两个版本的图片
            # 版本1：全部使用cross attention (use_cross_atten_mask=None)
            samples_with_ca = ode_fm_solver_sample(
                nnet_ema, 
                _n_samples=config.train.n_samples_eval, 
                _sample_steps=50, 
                context=_context, 
                token_mask=_token_mask, 
                image_latent=vis_input_image_latent,
                use_cross_atten_mask=None  # None表示全部使用cross attention
            )
            samples_with_ca_unpreprocessed = dataset.unpreprocess(samples_with_ca)  # [batch_size, 3, H, W], 范围 [0, 1]
            
            # 版本2：全部不使用cross attention
            mask_no_ca = torch.zeros(config.train.n_samples_eval, dtype=torch.bool, device=device)  # 全False
            samples_without_ca = ode_fm_solver_sample(
                nnet_ema, 
                _n_samples=config.train.n_samples_eval, 
                _sample_steps=50, 
                context=_context, 
                token_mask=_token_mask, 
                image_latent=vis_input_image_latent,
                use_cross_atten_mask=mask_no_ca  # 全False，不使用cross attention
            )
            samples_without_ca_unpreprocessed = dataset.unpreprocess(samples_without_ca)  # [batch_size, 3, H, W], 范围 [0, 1]
            ...
            if accelerator.is_main_process:
                ...
                # 确定设备：使用 samples_with_ca_unpreprocessed 的设备，或者 accelerator.device
                if len(samples_with_ca_unpreprocessed) > 0:
                    target_device = samples_with_ca_unpreprocessed[0].device
                else:
                    target_device = accelerator.device
                
                 # 准备拼接的图片列表：每行4张图片 [input, ground truth, generated_with_ca, generated_without_ca]
                all_images_for_grid = []
                
                for i in range(min(config.train.n_samples_eval, len(samples_with_ca_unpreprocessed))):
                    # 第1张：input image
                    ...
                    # 第3张：generated with cross attention
                    generated_with_ca = samples_with_ca_unpreprocessed[i]  # [C, H, W], 范围 [0, 1]
                    # 确保在正确的设备上
                    if generated_with_ca.device != target_device:
                        generated_with_ca = generated_with_ca.to(target_device)
                    if generated_with_ca.shape[1] != target_size or generated_with_ca.shape[2] != target_size:
                        generated_with_ca = generated_with_ca.unsqueeze(0)  # [1, C, H, W]
                        generated_with_ca = F.interpolate(generated_with_ca, size=(target_size, target_size), mode='bilinear', align_corners=False)
                        generated_with_ca = generated_with_ca.squeeze(0)  # [C, H, W]
                    all_images_for_grid.append(generated_with_ca)
                    
                    # 第4张：generated without cross attention
                    generated_without_ca = samples_without_ca_unpreprocessed[i]  # [C, H, W], 范围 [0, 1]
                    # 确保在正确的设备上
                    if generated_without_ca.device != target_device:
                        generated_without_ca = generated_without_ca.to(target_device)
                    if generated_without_ca.shape[1] != target_size or generated_without_ca.shape[2] != target_size:
                        generated_without_ca = generated_without_ca.unsqueeze(0)  # [1, C, H, W]
                        generated_without_ca = F.interpolate(generated_without_ca, size=(target_size, target_size), mode='bilinear', align_corners=False)
                        generated_without_ca = generated_without_ca.squeeze(0)  # [C, H, W]
                    all_images_for_grid.append(generated_without_ca)
                
                # 使用 make_grid 创建网格，每行4张图片，添加 padding 使图片更好看
                if all_images_for_grid:
                    # 将所有图片堆叠成 [n_images, C, H, W] 形状（都在GPU上）
                    images_tensor = torch.stack(all_images_for_grid, dim=0)  # [n_samples_eval*4, C, H, W]
                    # 使用 make_grid，nrow=4 表示每行4张图片：input, gt, with_ca, without_ca
                    final_image = make_grid(images_tensor, nrow=4, padding=2, pad_value=1.0)
                    ...
                    samples = make_grid(samples_with_ca_unpreprocessed, 5)
