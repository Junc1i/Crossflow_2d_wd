...
flags.DEFINE_integer("gate_warmup_steps", None, "Gate warmup steps: keep gate=1.0 for first N steps.")
flags.DEFINE_integer("gate_decay_steps", None, "Gate decay steps: cosine decay from 1.0 to gate_min_value over N steps.")
flags.DEFINE_float("gate_min_value", None, "Gate minimum value: final stable gate value after decay.")
...
def main(argv):
    ...
    # Cross-Attention Gate 衰减参数
    if FLAGS.gate_warmup_steps is not None:
        if hasattr(config.nnet.model_args, 'stage_configs') and len(config.nnet.model_args.stage_configs) > 0:
            config.nnet.model_args.stage_configs[0].gate_warmup_steps = FLAGS.gate_warmup_steps
    
    if FLAGS.gate_decay_steps is not None:
        if hasattr(config.nnet.model_args, 'stage_configs') and len(config.nnet.model_args.stage_configs) > 0:
            config.nnet.model_args.stage_configs[0].gate_decay_steps = FLAGS.gate_decay_steps
    
    if FLAGS.gate_min_value is not None:
        if hasattr(config.nnet.model_args, 'stage_configs') and len(config.nnet.model_args.stage_configs) > 0:
            config.nnet.model_args.stage_configs[0].gate_min_value = FLAGS.gate_min_value
