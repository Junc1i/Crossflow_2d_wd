...
class TransformerBlock(nn.Module):
    def __init__(self, config):
        ...
        if hasattr(config, 'use_cross_attention') and config.use_cross_attention:
            ...
            # 读取 gate 衰减参数
            self.gate_warmup_steps = getattr(config, 'gate_warmup_steps', 60000)
            self.gate_decay_steps = getattr(config, 'gate_decay_steps', 50000)
            self.gate_min_value = getattr(config, 'gate_min_value', 0.5)
        else:
    ...
    def forward(self, x, condition_embeds=None, condition_masks=None, pos=None, image_latent=None, training_step=None):
        if self.gradient_checking:
            return torch.utils.checkpoint.checkpoint(
                self._forward, x, condition_embeds, condition_masks, pos, image_latent, training_step
            )
        else:
            return self._forward(x, condition_embeds, condition_masks, pos, image_latent, training_step)
    
    def _forward(self, x, condition_embeds=None, condition_masks=None, pos=None, image_latent=None, training_step=None):
        ...
        if self.use_cross_attention and self.block1_cross is not None and image_latent is not None:
            gate = self._compute_cosine_decay_gate(training_step)
    ...
    def _compute_cosine_decay_gate(self, training_step):
        """
        分阶段余弦衰减:
        - 0-warmup_steps: gate = 1.0
        - warmup_steps到warmup_steps+decay_steps: gate从1.0余弦衰减到gate_min_value
        - warmup_steps+decay_steps之后: gate = gate_min_value
        """
        if training_step is None:
            # 推理时使用最小值
            return self.gate_min_value
        
        warmup_steps = self.gate_warmup_steps
        decay_steps = self.gate_decay_steps
        gate_min_value = self.gate_min_value
        
        if training_step < warmup_steps:
            # warmup阶段保持为1
            return 1.0
        elif training_step < warmup_steps + decay_steps:
            # decay阶段进行余弦衰减
            progress = (training_step - warmup_steps) / decay_steps
            # 余弦衰减: 从1.0衰减到gate_min_value
            gate = gate_min_value + (1.0 - gate_min_value) * 0.5 * (1.0 + torch.cos(torch.tensor(progress * 3.14159265359)))
            return gate
        else:
            # 稳定阶段保持为gate_min_value
            return gate_min_value
...
class Stage(nn.Module):
    ...
    def forward(self, images, lowres_skips = None, condition_context = None, condition_embeds = None, condition_masks = None, null_indicator=None, image_latent=None, training_step=None):
        ...
        for block in in_blocks:
            ...
            if isinstance(block, TransformerBlock):
                x = block(x, condition_embeds=condition_embeds, condition_masks=condition_masks, pos=pos, image_latent=image_latent_seq, training_step=training_step)
            else:
        ...
        if isinstance(mid_block, TransformerBlock):
            x = mid_block(x, condition_embeds=condition_embeds, condition_masks=condition_masks, pos=pos, image_latent=image_latent_seq, training_step=training_step)
        else:
...
class MRModel(nn.Module):
    ...
    def _forward(self, images, log_snr, condition_context = None, condition_text_embeds = None, condition_text_masks = None, condition_drop_prob = None, null_indicator=None, image_latent=None, training_step=None):
        if self.has_diff:
            ...
            for stage in self.stages:
                output, lowres_skips = stage(
                    ...
                    training_step=training_step  # 传递 training_step
                )
...
#        print("通过VAE之后的shape:",output.shape)
    ...
    def forward(self, x, t = None, log_snr = None, text_encoder=False, text_decoder=False, image_clip=False, shape=None, mask=None, null_indicator=None, image_latent=None, training_step=None):
        if text_encoder:
            ...
            return self._forward(images = x, log_snr = log_snr, null_indicator=null_indicator, image_latent=image_latent, training_step=training_step)
