...
class FlowMatching(nn.Module):  
    ...
    def p_losses_textVAE(
        ...
        noise=None,
        image_latent=None,
        input_visual_tokens=None,  # 新增：input image 的 visual tokens [B, 576, 2048]
        input_visual_mask=None,    # 新增：input image 的 mask [B, 576]
    ):
        """
...
#        print("x0_shape:",x0.shape)
        ############ loss for Text VE
        ############ loss for Text VE - 输入图像自对比 + Attention Pooling
        # VAE编码的输入图像latent
        text_features = x0 / x0.norm(dim=-1, keepdim=True)  # [B, latent_size]
        image_clip_features = nnet(input_visual_tokens, project_visual_tokens = True, mask=input_visual_mask)
        
        # 获取logit_scale（兼容DDP和非DDP）
        if hasattr(nnet, 'module'):
            logit_scale = nnet.module.logit_scale.exp()
        else:
            logit_scale = nnet.logit_scale.exp()
            
        image_features = image_clip_features / image_clip_features.norm(dim=-1, keepdim=True)  # [B, latent_size]
...
#        print("x_start shape:", x_start.shape)  # 输出目标 x_start 的维度
        ...
        prediction = nnet(x_noisy, log_snr = log_snr, null_indicator=null_indicator, image_latent=image_latent)
...
class ODEEulerFlowMatchingSolver(Solver):
    ...
    def sample(
        ...
        **kwargs,
        ...
        # 提取 image_latent 并确保传递给 sample_euler
        image_latent = kwargs.get("image_latent", None)
        # 将 image_latent 添加到 kwargs 中，以便传递给 sample_euler
